...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      timeout = setTimeout(tick, deleting ? 28 : 58)
    }
  ...
  return (
    <section id="home"
      className="hero-noise min-h-screen flex items-center pt-28 pb-16 px-6 sm:px-20 relative overflow-hidden">
      ...
      {/* Glow verde único */}
      <div className="absolute top-1/2 left-1/3 w-[600px] h-[600px] rounded-full pointer-events-none -translate-x-1/2 -translate-y-1/2
                      bg-[radial-gradient(circle,rgba(0,229,160,0.06)_0%,transparent_65%)]
      ...
      <div className="absolute left-6 sm:left-10 top-1/2 -translate-y-1/2 hidden sm:flex flex-col items-center gap-3">
        <div className="w-px h-20 bg-gradient-to-b from-transparent to-accent opacity-30" />
        <a href="https://github.com/Andre121314115" target="_blank" rel="noreferrer"
           className="font-mono text-[0.5rem] text-muted tracking-[4px] uppercase
      ...
      <div className="relative z-10 max-w-3xl w-full sm:ml-16">
        ...
        {/* Estado disponible */}
        <div className="inline-flex items-center gap-2 mb-10 px-3 py-1 border border-accent/20 bg-accent/5
                        opacity-0 animate-[fadeUp_0.8s_ease_0.2s_forwards]">
          <span className="w-[5px] h-[5px] rounded-full bg-accent animate-pulse" />
          <span className="font-mono text-[0.65rem] text-accent tracking-[2px] uppercase">
            {lang === 'es'
        ...
        {/* Nombre */}
        <h1 className="text-5xl sm:text-[6rem] font-extrabold leading-[0.95] mb-6
                       opacity-0 animate-[fadeUp_0.8s_ease_0.35s_forwards]">
        ...
        <p className="font-mono text-sm text-muted tracking-widest uppercase mb-4
          ...
          {lang === 'es' ? 'Ing. Sistemas · Backend · ML · Mobile' : 'Systems Eng. · Backend · ML · Mobile'}
        </p>

        {/* Typing */}
        <p className="font-mono text-base sm:text-lg text-accent min-h-[1.6em] mb-10
        ...
        <p className="text-muted leading-[1.85] max-w-lg text-[0.92rem] mb-10
                      opacity-0 animate-[fadeUp_0.8s_ease_0.7s_forwards]">
          {lang === 'es'
            ? 'Estudiante de Ingeniería de Sistemas en la Universidad Continental, décimo ciclo. Experiencia en redes, desarrollo web y análisis de datos con Python. Busco mi primera oportunidad como practicante o junior.'
            : 'Systems Engineering student at Universidad Continental, 10th semester. Experience in networks, web development and data analysis with Python. Looking for my first opportunity as an intern or junior dev.'}
        </p>

        {/* CTAs */}
        <div className="flex gap-4 flex-wrap opacity-0 animate-[fadeUp_0.8s_ease_0.85s_forwards]">
          <a href="#projects"
             className="px-8 py-3 font-mono font-bold text-sm tracking-wide
                        bg-accent text-bg hover:bg-accent2 transition-colors
                        [clip-path:polygon(0_0,calc(100%-12px)_0,100%_12px,100%_100%,12px_100%,0_calc(100%-12px))]">
            {lang === 'es' ? 'Ver proyectos' : 'View projects'}
          </a>
          <a href="#contact"
             className="px-8 py-3 border border-accent/40 text-accent font-mono text-sm tracking-wide
                        hover:bg-accent/8 hover:border-accent transition-all
                        [clip-path:polygon(0_0,calc(100%-12px)_0,100%_12px,100%_100%,12px_100%,0_calc(100%-12px))]">
          ...
          <a href="/cv/CV_AndreDeLaTorre.pdf" download
             className="px-8 py-3 border border-border text-muted font-mono text-sm tracking-wide
                        hover:border-accent/30 hover:text-text transition-all
                        [clip-path:polygon(0_0,calc(100%-12px)_0,100%_12px,100%_100%,12px_100%,0_calc(100%-12px))]">
            ⬇ CV
          </a>
        </div>
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2
                      font-mono text-[0.5rem] text-muted tracking-[4px] uppercase
